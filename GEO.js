document.addEventListener('DOMContentLoaded', function() {
    ...
    function calcularCustoMensalGEO(populacao) {
        let custoMensal;
    
        if (populacao <= 15000) {
            custoMensal = 1000;  
        } else if (populacao <= 30000) {
            custoMensal = populacao * 0.060;
        } else if (populacao <= 50000) {
            custoMensal = populacao * 0.050;
        } else if (populacao <= 100000) {
            custoMensal = populacao * 0.028;
    ...
    document.body.addEventListener('click', function(event) {
        const button = event.target.closest('#calcularButtonGEO');
        if (button) {
            ...
            const monthlyCost = calcularCustoMensalGEO(population);
    ...
    document.body.addEventListener('click', function(event) {
        const button = event.target.closest('.btnSistema[data-target="containerGEO"]');
        if (button) {
            showContainer('containerGEO');
        }
    });
